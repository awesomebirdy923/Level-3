...
public class CookieOrderTest {
	 ...
	 */
	class MasterOrder{
		private ArrayList<CookieOrder> cookieOrder;

		public MasterOrder(){
			this.cookieOrder = new ArrayList<CookieOrder>();
		}
		...
		public void addOrder(CookieOrder order){
			cookieOrder.add(order);
		}
		
		public int getTotalBoxes(){
			int total = 0;
			for (CookieOrder order : cookieOrder) {
		...
		public int removeVariety(String cookieVar){
			int removedLists = 0;
			for (CookieOrder order : cookieOrder) {
				if(order.getVariety() == cookieVar){
					cookieOrder.remove(this);
					removedLists+=order.getNumBoxes();
				}
			}
			return removedLists;
		}
